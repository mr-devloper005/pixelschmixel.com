...
const inputClass = 'h-12 rounded-[1rem] border border-[#d6deef] bg-[#f9fbff] px-4 text-base font-semibold text-[#173c78] outline-none transition placeholder:text-[#173c78]/35 focus:border-[#007979] focus:bg-white'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-[0.9rem] bg-[#173c78] px-6 text-sm font-black uppercase tracking-[0.12em] text-white shadow-sm transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1rem] px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
